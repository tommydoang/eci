...
public class Tab3ViewModel extends BaseViewModel<Tab3Navigator> {

    private List<DataProductTab3Response> productsList = new ArrayList<>();
    private List<DataBannerTab3Response> bannerList = new ArrayList<>();
    private final MutableLiveData<List<DataProductTab3Response>> tab3Data;
    private final ObservableList<DataProductTab3Response> tab3DataList = new ObservableArrayList<>();

    public Tab3ViewModel(DataManager dataManager, SchedulerProvider schedulerProvider) {
        super(dataManager, schedulerProvider);
        tab3Data = new MutableLiveData<>();
        getDataFromServer();
    }

    public MutableLiveData<List<DataProductTab3Response>> getImageLiveDataList() {
        return tab3Data;
    }

    public ObservableList<DataProductTab3Response> getTab3DataList() {
        return tab3DataList;
    }

    public void setDataToList(List<DataProductTab3Response> dataListApi){
        tab3DataList.clear();
        tab3DataList.addAll(dataListApi);
    }

    public List<DataBannerTab3Response> getAllBannerList(){
        return bannerList;
    }
    ...
    private void setProductModel(JSONArray jsonArray){
        try{
            Log.d("allProduc tes1 tab3", "MAIN VM " + jsonArray.length()+"");
            int jsonSize = jsonArray.length();
            for (int idx = 0; idx < jsonSize; idx++){
                JSONObject object = jsonArray.getJSONObject(idx);
                if (object.getString("id_category_head").equals("2")) {
...
//                        Log.d("skuProd tes1 tab2", objProd.getString("sku"));
                        ...
                        DataProductTab3Response dpr = new DataProductTab3Response();
...
//                        Log.d("longDesc tes1 2", objProd.getString("long_desc"));
                        ...
                        //Log.d("numStock tes1 tab2", objStock.getString("stock"));
            ...
            tab3Data.setValue(productsList);
        ...
        try{
            ...
            if (objResponseSize > 0) {
                for (int idx = 0; idx < objResponseSize; idx++) {
                    ...
                    if (subObjResponse.getString("meta_title").equals("2")) {
                        DataBannerTab3Response dbr = new DataBannerTab3Response();
