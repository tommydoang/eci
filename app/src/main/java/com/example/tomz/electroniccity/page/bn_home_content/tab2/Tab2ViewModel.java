...
public class Tab2ViewModel extends BaseViewModel<Tab2Navigator> {
    ...
    private List<DataProductTab2Response> productsList = new ArrayList<>();
    private List<DataBannerTab2Response> bannerList = new ArrayList<>();
    private final MutableLiveData<List<DataProductTab2Response>> tab2Data;
    private final ObservableList<DataProductTab2Response> tab2DataList = new ObservableArrayList<>();

    public Tab2ViewModel(DataManager dataManager, SchedulerProvider schedulerProvider) {
        super(dataManager, schedulerProvider);
        tab2Data = new MutableLiveData<>();
        getDataFromServer();
    }

    public MutableLiveData<List<DataProductTab2Response>> getImageLiveDataList() {
        return tab2Data;
    }

    public ObservableList<DataProductTab2Response> getTab2DataList() {
        return tab2DataList;
    }

    public void setDataToList(List<DataProductTab2Response> dataListApi){
        tab2DataList.clear();
        tab2DataList.addAll(dataListApi);
    }

    public List<DataBannerTab2Response> getAllBannerList(){
        return bannerList;
    }
    ...
    private void setProductModel(JSONArray jsonArray){
        try{
            Log.d("allProduc tes1 tab2", "MAIN VM " + jsonArray.length()+"");
            int jsonSize = jsonArray.length();
            for (int idx = 0; idx < jsonSize; idx++){
                JSONObject object = jsonArray.getJSONObject(idx);
                if (object.getString("id_category_head").equals("1")) {
                    ...
                    for (int idx2 = 0; idx2 < arrProdSize; idx2++) {
                        ...
                        DataProductTab2Response dpr = new DataProductTab2Response();
...
//                        Log.d("longDesc tes1 2", objProd.getString("long_desc"));
            ...
            tab2Data.setValue(productsList);
        ...
        try{
            ...
            if (objResponseSize > 0) {
                for (int idx = 0; idx < objResponseSize; idx++) {
                    ...
                    if (subObjResponse.getString("meta_title").equals("1")) {
                        DataBannerTab2Response dbr = new DataBannerTab2Response();
